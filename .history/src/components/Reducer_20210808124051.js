const reducer = (state, action) => {
  switch (action.type) {
    case "SET_PAPER":
      const data_SET_PAPER = state.data.map((item) => {
      ...
      return { ...state, cart: cart_SET_PAPER, data: data_SET_PAPER };

    case "SET_CHECKOUT":
      const data_SET_CHECKOUT = state.data.filter((item) => {
        if (item.id === action.payload.id) {
          return { ...item, checkout: action.payload.checkout };
        }
        return false;
      });
      return { ...state, cart: [...state.cart, data_SET_CHECKOUT] };
